...
static volatile u8 rand[10];
static volatile u8 key[6];
static volatile u8 ft[8];
static volatile u8 sb[8];
...
u8 GenEncryption()
{
	...
	for(idx = 0; idx < 7; idx++)
	{
		...
		for(i = 0; i < 6; i++)
		{
			test_ans[i] = ans[idx][i];
		}	
		for(i = 0; i < 10; i++)
		{
			test_s[i] = sboxes[0][rand[i]];
		}
	
		test_key[0] = ((ROR8((test_ans[0] ^ test_s[5]), (test_s[2] % 8)) - test_s[9]) ^ test_s[4]);
		test_key[1] = ((ROR8((test_ans[1] ^ test_s[1]), (test_s[0] % 8)) - test_s[5]) ^ test_s[7]);
		test_key[2] = ((ROR8((test_ans[2] ^ test_s[6]), (test_s[8] % 8)) - test_s[2]) ^ test_s[0]);
		test_key[3] = ((ROR8((test_ans[3] ^ test_s[4]), (test_s[7] % 8)) - test_s[3]) ^ test_s[2]);
		test_key[4] = ((ROR8((test_ans[4] ^ test_s[1]), (test_s[6] % 8)) - test_s[3]) ^ test_s[4]);
		test_key[5] = ((ROR8((test_ans[5] ^ test_s[7]), (test_s[8] % 8)) - test_s[5]) ^ test_s[9]);

		// Compare idx with key
		if(memcmp(test_key, (void*)key, 6) == 0) break;
	}
	if (idx == 7) return 0;

	// Generate encryption tables
	ft[0] = sboxes[idx + 1][key[4]] ^ sboxes[idx + 2][rand[3]];
	ft[1] = sboxes[idx + 1][key[2]] ^ sboxes[idx + 2][rand[5]];
	ft[2] = sboxes[idx + 1][key[5]] ^ sboxes[idx + 2][rand[7]];
	ft[3] = sboxes[idx + 1][key[0]] ^ sboxes[idx + 2][rand[2]];
	ft[4] = sboxes[idx + 1][key[1]] ^ sboxes[idx + 2][rand[4]];
	ft[5] = sboxes[idx + 1][key[3]] ^ sboxes[idx + 2][rand[9]];
	ft[6] = sboxes[idx + 1][rand[0]] ^ sboxes[idx + 2][rand[6]];
	ft[7] = sboxes[idx + 1][rand[1]] ^ sboxes[idx + 2][rand[8]];
	
	sb[0] = sboxes[idx + 1][key[0]] ^ sboxes[idx + 2][rand[1]];
	sb[1] = sboxes[idx + 1][key[5]] ^ sboxes[idx + 2][rand[4]];
	sb[2] = sboxes[idx + 1][key[3]] ^ sboxes[idx + 2][rand[0]];
	sb[3] = sboxes[idx + 1][key[2]] ^ sboxes[idx + 2][rand[9]];
	sb[4] = sboxes[idx + 1][key[4]] ^ sboxes[idx + 2][rand[7]];
	sb[5] = sboxes[idx + 1][key[1]] ^ sboxes[idx + 2][rand[8]];
	sb[6] = sboxes[idx + 1][rand[3]] ^ sboxes[idx + 2][rand[5]];
	sb[7] = sboxes[idx + 1][rand[2]] ^ sboxes[idx + 2][rand[6]];
...
{   
	...
	// Transfer encryption data to tables
	for(i = 0; i < 10; i++)
	{
		rand[9 - i] = buf[i];
	}
    ...
    // Generate encryption tables
	return GenEncryption();
}

u8 Encrypt(u8 addr, u8 data)
{
    return (data - ft[addr % 8]) ^ sb[addr % 8];
}

u8 Decrypt(u8 addr, u8 data)
{
    return (data ^ sb[addr % 8]) + ft[addr % 8];
}
